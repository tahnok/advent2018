...
fn main() {
    ...
    for (&id, ref times) in &guards_and_times {
        let mut sleep = 0;
        for (_, count) in times.iter() {
           sleep += count;
        }
        if sleep > sleepiest_time {
            sleepiest_time = sleep;
            sleepiest_guard = id;
        }
    }

    let mut sleepiest_hour_count = 0;
    let mut sleepiest_hour = 0;
    for (&time, &count) in guards_and_times.get(&sleepiest_guard).unwrap() {
        if count > sleepiest_hour_count {
            sleepiest_hour = time;
            sleepiest_hour_count = count;

        }
    }
    println!("{}", sleepiest_guard * sleepiest_hour);
